...
cw.add_update("player_controller", function()
	...
	elseif not __window.debug_window and not __window.is_editor then
		...
		if input.key_down(input.K_s) and not Raycast.cone(_player.center_position + Vector2(0,16), 45, 90, 25, c_solid) then
			_player.transform.position.y = move_amt + _player.transform.position.y
		end
		if input.key_down(input.K_w) and not Raycast.cone(_player.center_position + Vector2(0,16), 45, 270, 25, c_solid) then
			_player.transform.position.y = _player.transform.position.y - move_amt
		end
		if input.key_down(input.K_d) and not Raycast.cone(_player.center_position + Vector2(0,16), 56, 0, 25, c_solid) then
			_player.transform.position.x = move_amt + _player.transform.position.x
		end
		if input.key_down(input.K_a) and not Raycast.cone(_player.center_position + Vector2(0,16), 56, 180, 25, c_solid) then
