...
struct RectCollider2d {
	...
	// Rect according to the camera. If this is associated with a sprite, use Sprite::rect!
...
SDL_Rect RectCollider2d::recalc(RectCollider2d out) {
	...
	if (out.centered || out.layer == COL_CENTER) {
		m_out.x = ((int)((Camera::GetInstance()->m_Viewport.x * 0.5f) - (out.ws_rect->w * 0.5f)));
		m_out.y = ((int)((Camera::GetInstance()->m_Viewport.y * 0.5f) - (out.ws_rect->h * 0.5f)));
	}
	else {
		m_out.x = -Camera::GetInstance()->m_target->x + out.ws_rect->x + (Camera::GetInstance()->m_Viewport.x * 0.5f);
		m_out.y = -Camera::GetInstance()->m_target->y + out.ws_rect->y + (Camera::GetInstance()->m_Viewport.y * 0.5f);
	}
	return m_out;
}

void RectCollider2d::recalc() {
	*this->rect = recalc(*this);
}

RectCollider2d* RectCollider2d::lu_new_fromr(VectorRect r, int __layer) {
...
RectCollider2d* RectCollider2d::lu_new_fromi(int x, int y, int w, int h, int __layer) {
	RectCollider2d* r_re = new RectCollider2d();
	*r_re->rect = { x, y, w, h };
...
RectCollider2d* RectCollider2d::lu_new_fromic(int x, int y, int w, int h, int __layer) {
	RectCollider2d* r_re = new RectCollider2d();
	*r_re->rect = { x, y, w, h };
	*r_re->ws_rect = { x, y, w, h };
	r_re->recalc();
...
void RectCollider2d::c_collider_hit(RectCollider2d* o) {
	//if (SUCCEEDED(&lua_colhit)) { lua_colhit.execute(o); }
	OnColliderHit(this, o);
}

RectCollider2d::RectCollider2d(SDL_Rect* Rect, int Layer) {
...
const int hrtocol(std::string id) {
	...
	if (alib_streq(id, "COL_ITM")) { return COL_ITM; }
...
MeshCollider2d::MeshCollider2d(std::vector<MeshLine*> Lines, int Layer) {
...
MeshCollider2d* MeshCollider2d::lu_new(std::vector<MeshLine*> __lines) {
...
SDL_Rect MeshLine::bounding_boxi(int sx, int sy, int ex, int ey) {
	VectorRect _r = {
		alib_min(sx, ex) - 2,
		alib_min(sy, ey) - 2,
...
SDL_Rect MeshLine::bounding_box(MeshLine m) {
	return bounding_boxi(m.start.x, m.start.y, m.end.x, m.end.y);
}

Vector2 MeshLine::midpoint() {
	return { (start.x + end.x) * 0.5f, (start.y + end.y) * 0.5f };
}

Vector2 MeshLine::midpoint(Vector2 s, Vector2 e) {
	return { (s.x + e.x) * 0.5f, (s.y + e.y) * 0.5f };
}

MeshLine* MeshLine::lu_new(double x1, double y1, double x2, double y2, int _layer) {
...
MeshLine::MeshLine(Vector2 _s, Vector2 _e) {
	this->start = _s; this->end = _e;
}

MeshLine::MeshLine(double x1, double y1, double x2, double y2) {
	this->start = { x1, y1 };
	this->end = { x2, y2 };
}
...
bool Raycast2D::RectIntersects(SDL_Rect* r1, SDL_Rect* r2) {
	SDL_Rect _r = {};
...
bool Raycast2D::pointRect(SDL_Rect* rect, Vector2 pos)
{
...
bool Raycast2D::lineRect(Vector2 start, Vector2 end, SDL_Rect* rect) {
...
bool Raycast2D::TestLineImplV(Vector2 start1, Vector2 end1, Vector2 start2, Vector2 end2) {
	return lineLine(start1.x, start1.y, end1.x, end1.y, start2.x, start2.y, end2.x, end2.y);
}

bool Raycast2D::TestLineImpl(MeshLine first, MeshLine second) {
...
bool Raycast2D::Test(Vector2 start, Vector2 end, int layer) {
	...
	for (unsigned int li = 0; li < MeshCollider2d::_mGlobalColArr.size(); li++) {
		...
		for (size_t lo = 0; lo < m->lines.size(); lo++) {
			...
			Vector2 s = l.start - *Camera::GetInstance()->m_target + (Camera::GetInstance()->m_Viewport * 0.5f);
			Vector2 e = l.end - *Camera::GetInstance()->m_target + (Camera::GetInstance()->m_Viewport * 0.5f);
...
bool Raycast2D::TestExcept(Vector2 start, Vector2 end, int layer) {
	RaycastHit::hasHit = false;
	//SDL_SetRenderDrawColor(__phys_internal_renderer, 255, 64, 255, 255);
	//SDL_RenderDrawLine(__phys_internal_renderer, start.x, start.y, end.x, end.y);
	for (unsigned int i = 0; i < RectCollider2d::_mGlobalColArr.size() && !RaycastHit::hasHit; i++) {
...
bool Raycast2D::TestRect(Vector2 start, Vector2 end, int layer) {
	RaycastHit::hasHit = false;
	//SDL_SetRenderDrawColor(__phys_internal_renderer, 255, 64, 255, 255);
	//SDL_RenderDrawLine(__phys_internal_renderer, start.x, start.y, end.x, end.y);
	for (unsigned int i = 0; i < RectCollider2d::_mGlobalColArr.size() && !RaycastHit::hasHit; i++) {
...
bool Raycast2D::TestRectExcept(SDL_Rect* rect, int layer, RectCollider2d* ignored_col) {
	RaycastHit::hasHit = false;
	//SDL_SetRenderDrawColor(__phys_internal_renderer, 255, 64, 255, 255);
	//SDL_RenderDrawLine(__phys_internal_renderer, start.x, start.y, end.x, end.y);
	for (unsigned int i = 0; i < RectCollider2d::_mGlobalColArr.size() && !RaycastHit::hasHit; i++) {
		...
		if (_thiscol->layer & layer || layer == -1) {
			...
			if (o) {
				RaycastHit::object = _thiscol;
				RaycastHit::hasHit = true;
				RaycastHit::start = { (double)_thiscol->rect->x, (double)_thiscol->rect->y };
				RaycastHit::end = RaycastHit::start + Vector2{ (double)_thiscol->rect->w, (double)_thiscol->rect->h };
				*(&RaycastHit::col_id) = _thiscol->id;
				RaycastHit::layer = _thiscol->layer;
				return true;
			}
...
bool Raycast2D::TestCircle(Vector2 start, int diameter, int layer) {
...
bool Raycast2D::TestCone(Vector2 start, int angle, int offset, int dist, int layer) {
...
bool Raycast2D::TestAnyCone(Vector2 start, int angle, int offset, int dist) {
	return TestCone(start, angle, offset, dist, -1);
}

bool Raycast2D::TestAnyLine(Vector2 start, Vector2 end) {
	return Test(start, end, -1);
}

bool Raycast2D::TestAnyCircle(Vector2 start, int diameter) {
	return TestCircle(start, diameter, -1);
}

bool Raycast2D::TestConeExcept(Vector2 start, int angle, int offset, int dist, int layer) {
	for (float a = 0 - angle + offset; a < angle + offset; a += (360.0f + angle + offset) / 512.0f) {
		Vector2 i(
			start.x + (dist * cos(alib_deg2rad(a))),
			start.y + (dist * sin(alib_deg2rad(a)))
...
bool Raycast2D::TestCircleExcept(Vector2 start, int diameter, int layer) {
	for (float a = 0; a < 360; a += (360.0f / diameter) / 512.0f) {
		Vector2 i(
			start.x + (diameter * cos(alib_deg2rad(a))),
			start.y + (diameter * sin(alib_deg2rad(a)))
