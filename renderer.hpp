...
struct SpriteAnimationMeta {
	...
	void set_loop_func(luabridge::LuaRef r) {
		OnLoopOrReset = r;
	}
	SpriteAnimationMeta() { }
...
struct ParticleEffects {
	...
	static inline stub RAIN = { .uv{7,0,3,3}, .id = "cw::rain", .lifetime = 10, .size = {12,12}, .randomness={100,0}, .angle = Vector2::parse_angle_vec(Vector2::down + (Vector2::left * 0.2f)),
		.meta = SpriteAnimationMeta(3, 0.2f)
		};
...
inline void Sprite::Update() {
	if (meta.frames > 0 && (meta.animate || meta.animate_once)) {
		...
		if (meta.CURRENT_FRAME == meta.frames) {
			meta.OnLoopOrReset();
		}
	...
	if (center) {
		_offset = Vector2(
			Camera::GetInstance()->m_Viewport.x * 0.5f,
			Camera::GetInstance()->m_Viewport.y * 0.5f
		) + _cameraViewOffset;
	}
	else {
		_offset = (
			(-*Camera::GetInstance()->m_target + transform.position) + Camera::GetInstance()->m_Viewport * 0.5f
			);
	}
...
inline Sprite::Sprite(const char* filename, SDL_Rect spriteuv) {
	name = filename;
	uv = spriteuv;
	layer = 0;
	_mglobalspritearr.push_back(this);
...
inline bool Sprite::eraseElementByID(std::string id) {
...
inline Sprite* Sprite::getElementByID(std::string id) {
	for (int i = 0; i < _mglobalspritearr.size(); i++) {
		Sprite* _s = _mglobalspritearr[i];
		if (_s->isPrivate) { continue; }
		if (alib_streq(id, _s->identifier.c_str())) {
...
inline void Sprite::insertElementWithID(std::string id, Sprite* s) {
	s->setID(id);
	_mglobalspritearr.push_back(s);
}

inline Sprite* Sprite::copy() {
	Sprite* s = new Sprite();
...
inline ParticleEffect* ParticleEffect::getElementByID(std::string id) {
...
inline ParticleEffect::ParticleEffect(ParticleEffects::stub m_particle, float m_amt) {
	if (!_m_ParticleImage) {
...
inline void ParticleEffect::append(int count) {
	amt += count;
...
inline void ParticleEffect::pop(int count) {
	amt -= count;
	for (int i = 0; i < count; i++) {
...
inline void ParticleEffect::refresh() {
...
inline void ParticleEffect::Render() {
	for (int i = 0; i < particles.size(); i++) {
...
inline void BatchRenderer::Render() {
